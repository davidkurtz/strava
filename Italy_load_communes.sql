...
BEGIN
    ...
    l_blob := DBMS_CLOUD.GET_OBJECT
              (credential_name => 'OBJECT_STORE_CRED'
              ,object_uri      => 'https://objectstorage.uk-london-1.oraclecloud.com/n/lrp1qmpxv8ea/b/bucket-gofaster1/o/it2.json'
              );
    --l_clob := CAST(l_blob AS CLOB);
    -- Insert into table
    INSERT INTO strava.stage_geo_data(name, geo_json)
    VALUES ('Italy Communes', l_blob);
...
BEGIN 
  SELECT geo_json INTO l_clob FROM stage_geo_data WHERE name = 'Italy Communes';
  ...
  FOR i IN 0 .. j_features.get_size - 1 LOOP
	...
	ELSE
	  ...
	  l_uqid   := j_properties.get_string('id')||i;
	...
	, mbr, num_pts, area)
	VALUES
	('MUNC', 380000+l_id, 6, l_uqid, l_name, l_geom
...
set area_number = area_number + 380000
where area_number < 1000
/
...
SELECT activity_id, start_date_local, name, description, area_list, processing_status
FROM   activities
WHERE  activity_id = 14776937278
