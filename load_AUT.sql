...
#unzip /vagrant/files/AUT_adm.zip -d /tmp/strava/
...
select distinct engtype_1 from AUT_adm1;
select distinct engtype_2 from AUT_adm2;

insert into my_area_codes values ('STAT','State',5);
insert into my_area_codes values ('DIS','District',6);
insert into my_area_codes values ('CITS','Statutory City',6);

update AUT_adm2
set engtype_2 = 'Statutory City'
where engtype_2 IS NULL;
...
, p.uqid parent_uqid
, c.area_level
from AUT_adm1 x, my_area_codes c, p
...
, p.uqid parent_uqid
, 6 area_level
from AUT_adm2 x, my_area_codes c, p
