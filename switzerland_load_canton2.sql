REM Switzerland_load_canton2.sql
...
delete from stage_geo_data where name = 'Switzerland';
DECLARE 
  l_url VARCHAR2(1000)
  := 'https://labs.karavia.ch/swiss-boundaries-geojson/geojson/2020/swissBOUNDARIES3D_1_3_TLM_KANTONSGEBIET.geojson';
...
BEGIN
  ...
  --strava_sdo.parse_geojson(l_clob);
    ...
    VALUES ('Switzerland Cantons', l_clob);
...
BEGIN 
  SELECT geo_json INTO l_clob FROM stage_geo_data WHERE name = 'Switzerland Cantons';
  ...
  FOR i IN 0 .. j_features.get_size - 1 LOOP
	...
	ELSE
	  ...
	  --strava_http.print_clob(l_clob);
	  l_id               := 765000 + 10*j_properties.get_number('KANTONSNUM') 
	                               +    j_properties.get_number('KT_TEIL');
	...
	, mbr, num_pts, area)
	VALUES
	('CANT', l_id, 5, 'CHC'||l_id, l_name, l_geom
...
, parent_area_number = 1159320491
, parent_uqid = 'NE1159320491'
...
INSERT INTO stage_my_areas
(area_code, area_number, uqid, area_level, name, geom)
SELECT  area_code, TRUNC(area_Number,-1), 'CHC'||TRUNC(area_Number,-1), min(area_level), name
--,     count(*) num_parts
,       SDO_AGGR_UNION(SDOAGGRTYPE(geom, 0.005)) AS merged_geom
FROM   stage_my_areas
WHERE  area_code = 'CANT'
AND    mod(area_number,10)>0
GROUP BY area_code, TRUNC(area_Number,-1), name
HAVING COUNT(*) > 1
;
...
/
UPDATE stage_my_areas
set parent_area_code = 'CANT'
, parent_area_number = TRUNC(area_Number,-1)
, parent_uqid = 'CHC'||TRUNC(area_Number,-1)
--, mbr = sdo_geom.sdo_mbr(geom)
--, area = SDO_GEOM.SDO_AREA(geom, 0.001,'unit=sq_km')
--, num_pts = SDO_UTIL.GETNUMVERTICES(geom)
WHERE mod(area_number,10) > 0
...
where 1=1
and a.area_code = 'SOVC' and a.area_number IN(1159320491)
...
WHERE area_code = 'CANT'
AND area_number BETWEEN 1 and 26
/
