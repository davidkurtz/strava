REM northernireland_load_countiess.sql
REM OSNI Open Data - Largescale Boundaries - County Boundaries
REM https://admin.opendatani.gov.uk/dataset/osni-open-data-largescale-boundaries-county-boundaries/
REM see also OSNI county https://admin.opendatani.gov.uk/dataset/osni-open-data-largescale-boundaries-local-government-districts-1993
REM see also OSNI townlands https://admin.opendatani.gov.uk/dataset/osni-open-data-50k-boundaries-townlands
REM see also OSNI AONB https://admin.opendatani.gov.uk/dataset/areas-of-outstanding-natural-beauty
...
DECLARE 
  l_url VARCHAR2(1000) :=
'https://admin.opendatani.gov.uk/dataset/1e70ee82-94ec-4adb-b78a-f1f1b44a2be3/resource/88369ae9-2726-4d49-84e5-f202043f9de2/download/osni_open_data_50k_boundaries_ni_counties.geojson';
--'https://admin.opendatani.gov.uk/dataset/d0385f2d-6beb-4aff-87dc-f1bf357d792d/resource/108d8567-3ec7-4403-8912-bcc6233bf361/download/osni_open_data_largescale_boundaries_county_boundaries.geojson';
  l_clob CLOB;
BEGIN
...
BEGIN 
  ...
  FOR i IN 0 .. j_features.get_size - 1 LOOP
	...
	ELSE
	  ...
	  strava_http.print_clob(l_clob);
	  --l_id               := j_properties.get_string('COUNTY_ID');
	  l_id               := j_properties.get_string('OBJECTID');
	  l_name             := initcap(j_properties.get_string('CountyName'));
	...
	('CTY', l_id, 5, 'NICTY'||l_id, l_name, l_geom, sdo_geom.sdo_mbr(l_geom), l_num_pts
