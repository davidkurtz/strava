REM Switzerland_load_regions.sql
REM https://simplemaps.com/gis/country/ch
...
----------------------------------------------------------------------------------------------------
--load json FROM https://simplemaps.com/gis/country/ch
----------------------------------------------------------------------------------------------------
clear screen
set serveroutput on 
set timi on
...
BEGIN
    ...
    l_blob := DBMS_CLOUD.GET_OBJECT
              (credential_name => 'OBJECT_STORE_CRED'
              ,object_uri      => 'https://objectstorage.uk-london-1.oraclecloud.com/n/lrp1qmpxv8ea/b/bucket-gofaster1/o/it1.json'
              );
    --l_clob := CAST(l_blob AS CLOB);
    -- Insert into table
    INSERT INTO strava.stage_geo_data(name, geo_json)
    VALUES ('Switzerland Regions', l_blob);
...
BEGIN 
  SELECT geo_json INTO l_clob FROM stage_geo_data WHERE name = 'Switzerland Regions';
  ...
  FOR i IN 0 .. j_features.get_size - 1 LOOP
	...
	ELSE
	  ...
	  strava_http.print_clob('j_properties='||l_clob);

	  l_uqid   := j_properties.get_string('id');
	  l_id     := REGEXP_SUBSTR(l_uqid,'[[:digit:]]+',1,1);
      l_name := /*initcap_placename*/(j_properties.get_string('name'));
      dbms_output.put_line(l_id||', '||l_uqid||', '||l_name);
	END IF;
	...
	IF j_geometry IS NULL THEN
	  ...
	  l_geom := NULL;
	ELSE
	  --l_regionid         := j_geometry.get_string('ID');
	  ...
	  EXCEPTION 
	    WHEN e_json_syntax_error THEN
		  dbms_output.put_line(sqlerrm||' during parse of '||l_name||' ('||l_regionid||'). Switch to own function.' );
	...
	, mbr, num_pts, area)
	VALUES
	('REG', l_id, 5, l_uqid, l_name, l_geom
...
or parent_area_number = 88
/

--reparent these? GEOS 1159320927,1159320929,1159320921,1159320925,1159320923
delete from my_area where area_code = 'GEOS' and area_number IN(1159320929,1159320921,);
update stage_my_areas set name = 'Sicily' where name = 'Sicilia';
update stage_my_areas set name = 'Sardinia' where name = 'Sardegna';
...
BEGIN
  FOR i IN (
    ...
    area_hsearch(i.area_code, i.area_number, 'SOVC',380, 'A', 0);
	COMMIT;
  END LOOP;
END;
/
...
BEGIN
  ...
  FOR i IN(
   WITH x AS (
   SELECT m.area_code, m.area_number, m.name
   ,      new.area_code new_area_code, new.area_number new_area_number, new.name new_name
   ,      sdo_geom.sdo_area(SDO_GEOM.sdo_intersection(m.geom,new.geom,0.001), unit=>'unit=sq_km') new_area
   ,      CASE WHEN m.geom  IS NOT NULL AND new.geom IS NOT NULL THEN sdo_geom.sdo_area(SDO_GEOM.sdo_intersection(m.geom,new.geom,0.001), unit=>'unit=sq_km') 
		  END geom_area
   FROM   my_areas m
   ,      my_areas new
   ...
   ORDER BY geom_area desc nulls last fetch first 1 rows only
    ...
    IF i.geom_area/i.new_area >= .99 THEN
      dbms_output.put_line(l_pad||'Found '||i.area_code||'-'
                                ||i.area_number||':'||i.name
		  	 	||', area '||i.new_area||' km^2'
			 	||', intersection area '||i.geom_area||' km^2 ('||100*i.geom_area/i.new_area||'%)'
				);
	  IF i.area_code = p_searchfor_area_code AND i.area_number = p_searchfor_area_number then
...
BEGIN
  FOR i IN (
    ...
    AND   area_number IN(1159320925,1159320923)
    --AND parent_area_number IS NULL
    --FETCH FIRST 10 ROWS ONLY
  ) LOOP
    area_hsearch(i.area_code, i.area_number, 'SOVC',380, 'A', 0);
...
where 1=1
and a.area_code = 'SOVC' and a.area_number = 380
...
/
----------------------------------------------------------------------------------------------------
--reset all the italian areas
----------------------------------------------------------------------------------------------------
update activities set processing_status = 3 
where activity_id IN(
select aa.activity_id
from my_areas m
  inner join activity_areas aa on aa.area_code = m.area_code and aa.area_number = m.area_number
where m.parent_area_code = 'SOVC' and m.parent_area_number = 380)
/
commit;

--exec strava_sdo.activity_area_hsearch(14776937278);
exec strava_sdo.activity_hsearch_upd(14776937278);

SELECT activity_id, start_date_local, name, distance_km, area_list, processing_status
FROM   activities
WHERE  activity_id =c 14776937278
or area_list like '%Switzerland%'
order by 2 desc
/
