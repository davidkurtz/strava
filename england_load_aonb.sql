REM england_load_aonb.sql
REM https://files.planning.data.gov.uk/dataset/area-of-outstanding-natural-beauty.geojson
...
CREATE TABLE stage_my_areas (
	...
	WIKIDATAID VARCHAR2(20 CHAR) NOT NULL , 
...
DECLARE 
  ...
  l_wikidataid  VARCHAR2(100 char);
  l_num_pts     INTEGER;
  l_offset      NUMBER;
...
BEGIN 
  ...
  FOR i IN 0 .. j_features.get_size - 1 LOOP
	...
	ELSE
	  ...
	  strava_http.print_clob(l_clob);
	  l_id               := j_properties.get_string('entity');
	  l_name             := initcap(j_properties.get_string('name'));
	  l_wikidataid       := initcap(j_properties.get_string('wikidata'));
	...
	(area_code, area_number, area_level, uqid, name, wikidataid, geom, mbr, num_pts)
    VALUES
	('AONB', l_id, 5, 'ENGAONB'||l_id, l_name, l_wikidataid, l_geom, sdo_geom.sdo_mbr(l_geom), l_num_pts
	)
	;
  END LOOP;
  Commit;
...
select * from stage_my_areas order by 1,2;

----------------------------------------------------------------------------------------------------
--select * from my_areas where name = 'United Kingdom';
--UK: SOV	1159320701
--England: GEOU	1159320743
--NI: GEOU:1159320745
--select * from my_areas where name = 'Isle of Wight';
--select * from my_areas where name like '%Scilly%' ;
----------------------------------------------------------------------------------------------------

UPDATE stage_my_areas s
set s.parent_area_code = 'GEOU'
, s.parent_area_number = 1159320743
, s.parent_uqid = 'NE1159320743'
, matchable = 1
, area_level = 5
, name = initcap(name)
/

UPDATE stage_my_areas s --Isle of Wight
set s.parent_area_code = 'UTA'
, s.parent_area_number = 129144
, s.parent_uqid = 'E06000046'
WHERE area_code = 'AONB'
and   area_number = 1000016
/

UPDATE stage_my_areas s --Isle of Scilly
set s.parent_area_code = 'UTA'
, s.parent_area_number = 129071
, s.parent_uqid = 'E06000053'
WHERE area_code = 'AONB'
and   area_number = 1000017
/
...
where parent_area_code = 'GEOU' AND parent_area_number = 1159320743
/
...
/
----------------------------------------------------------------------------------------------------
-- force area recalculate all AONBs
----------------------------------------------------------------------------------------------------
update activities
