REM swisstopo_load_country.sql
REM https://www.swisstopo.admin.ch/de/landschaftsmodell-swissboundaries3d as shapefile
REM convert with mapshaper.org
...
DROP TABLE admin.stage_my_areas PURGE;
...
----------------------------------------------------------------------------------------------------
--load json FROM https://www.swisstopo.admin.ch/de/landschaftsmodell-swissboundaries3d as shapefile
----------------------------------------------------------------------------------------------------
clear screen
set serveroutput on 
set timi on
...
REM new bucket token g[hvGyoe94jvh;ahk;U9

begin
  dbms_cloud.update_credential(
    credential_name => 'OBJECT_STORE_CRED',
    attribute       => 'username', 
    value           => 'info@go-faster.co.uk');
end;
/
BEGIN
  dbms_cloud.update_credential(
...
BEGIN
    ...
    l_blob := DBMS_CLOUD.GET_OBJECT
              (credential_name => 'OBJECT_STORE_CRED'
              ,object_uri      => 'https://objectstorage.uk-london-1.oraclecloud.com/n/lrp1qmpxv8ea/b/bucket-gofaster1/o/swissBOUNDARIES3D_1_5_TLM_LANDESGEBIET.json'
              );
    --l_clob := CAST(l_blob AS CLOB);
    -- Insert into table
    INSERT INTO strava.stage_geo_data(name, geo_json)
    VALUES ('Switzerland', l_blob);
...
BEGIN 
  SELECT geo_json INTO l_clob FROM stage_geo_data WHERE name = 'Switzerland';
  ...
  j_geometries := j_root.get_array('geometries');
  --l_srid     := REGEXP_SUBSTR(j_crs.get_object('properties').get_string('name'),'[^:]+',1,2);
  l_srid := 2056; --LV95 LN02 Horizontal EPSG:2056, Vertical EPSG:5728, Compound EPSG:7416
  FOR i IN 0 .. j_geometries.get_size - 1 LOOP
    j_feature := TREAT(j_geometries.get(i) AS JSON_OBJECT_T);
